...
class DepthNetwork(nn.Module):
    ...
    def __init__(self, cfg, **kwargs):
        ...
        for l in range(0, self.num_layers - 1):
            ...
            # NOTE Not sure about this initialization
...
class NeuralNetwork(nn.Module):
    ...
    def __init__(self, cfg, **kwargs):
        ...
        # geo network
        dims_geo = [dim_embed] + [hidden_size if i in self.skips else hidden_size for i in
                                  range(0, self.num_layers)] + [self.feat_size + 1]
        ...
        self.tanh = nn.Tanh()
        self.sigmoid = nn.Sigmoid()
        self.depth_network = DepthNetwork(cfg)
    ...
    def forward(self, p, ray_d=None, only_occupancy=False, return_logits=False, return_addocc=False, noise=False,
                **kwargs):
        x = self.infer_occ(p)
        if only_occupancy:
