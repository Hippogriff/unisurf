...
class Renderer(nn.Module):
    ...
    def forward(self, pixels, camera_mat, world_mat, scale_mat,
        ...
        elif rendering_technique == 'depth':
            out_dict = self.depth(
                pixels, camera_mat, world_mat,
                scale_mat, it=it, add_noise=add_noise, eval_=eval_
            )
    ...
    def depth(self, pixels, camera_mat, world_mat, scale_mat, add_noise=False, it=100000, eval_=False):
        # Get configs
        ...
        # NOTE Get mask for predicted depth
        ...
        # NOTE For rays with invalid depth values
    ...
    def unisurf(self, pixels, camera_mat, world_mat,
        ...
        # import trimesh;
        # import trimesh; pcd = trimesh.PointCloud(p_proposal.data.cpu().numpy()[0]); pcd.scene().show()
        # pcd.scene().show()
        ...
        import ipdb;
        ipdb.set_trace()
        intersect_points = camera_world + depth_intersect * ray_vector

        import trimesh;
        import trimesh; pcd = trimesh.PointCloud(intersect_points.data.cpu().numpy()); pcd.scene().show()
        pcd.scene().show()
    ...
    def onsurf_renderer(self, pixels, camera_mat, world_mat,
        ...
        with torch.no_grad():
            d_i = self.call_depth_function(camera_world, ray_vector, self.model,
                                           it=8, n_steps=[int(512), int(512) + 1], check_unitsphere_intersection=True,
                                           rad=rad)
